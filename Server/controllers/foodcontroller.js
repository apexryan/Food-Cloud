...
//=============================
//6.Admin: Mark food post as unavailable
//=============================

const markAsUnavailable = async (req, res) => {
  try {
    const { id } = req.params;
    console.log(`🔒 Admin marking food post as unavailable: ${id}`);

    // Check if user is admin
    if (!req.user || req.user.role !== "admin") {
      return res.status(403).json({
        success: false,
        message: "Only administrators can mark posts as unavailable",
      });
    }
    ...
    // Check if food is already in a final state
    if (["delivered", "canceled"].includes(foodPost.status)) {
      return res.status(400).json({
        success: false,
        message:
          "Cannot modify food post that is already delivered or canceled",
      });
    }
    ...
    console.log("Food post marked as unavailable successfully");

    res.status(200).json({
      success: true,
      message: "Food post marked as unavailable successfully",
      data: foodPost,
    });
  } catch (error) {
    console.error("Error marking food post as unavailable:", error);
    ...
    res.status(500).json({
      success: false,
      message: "Failed to mark food post as unavailable",
      error: error.message,
    });
  }
};
...
//=============================
//7.Admin: Mark food post as available
//=============================

const markAsAvailable = async (req, res) => {
  try {
    const { id } = req.params;
    console.log(`✅ Admin marking food post as available: ${id}`);

    // Check if user is admin
    if (!req.user || req.user.role !== "admin") {
      return res.status(403).json({
        success: false,
        message: "Only administrators can mark posts as available",
      });
    }
    ...
    console.log("Food post marked as available successfully");

    res.status(200).json({
      success: true,
      message: "Food post marked as available successfully",
      data: foodPost,
    });
  } catch (error) {
    console.error("Error marking food post as available:", error);
    ...
    res.status(500).json({
      success: false,
      message: "Failed to mark food post as available",
      error: error.message,
    });
  }
};
